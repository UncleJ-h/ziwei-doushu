...
// round 4 M-1 卖点接通：把死代码 patterns.ts (1118 行 34 detectors) + sihua.ts 年干四化接进 generateChart 返回值。
// 倪师立场仍然守住：sihua.ts 里的 getDaXianSiHua / getLiuNianSiHua / detectSelfSihua / 来因宫 等飞星派函数不调用，
// 只用 getSiHuaByStem(本命年干) 这一个静态四化。
...
// ─── 亮度映射 ────────────────────────────────────────────────────
// round 5 NR5-2：把 iztro 原始 7 级亮度（庙/旺/得/利/平/不/陷）映射到 3 级压缩
// （bright/normal/dim），向后兼容老消费者；同时通过 mapBrightnessRaw 透传原始 7 级
// 字符串，供严格命理对照（生产链路读 Star.brightnessRaw 拿原始值，不再绕过 iztro）。
...
// round 5 NR5-2：透传 iztro 7 级原始亮度（若不在白名单则返回 undefined）。
const BRIGHTNESS_RAW_SET = new Set(["庙", "旺", "得", "利", "平", "不", "陷"]);
function mapBrightnessRaw(
  b?: string,
): "庙" | "旺" | "得" | "利" | "平" | "不" | "陷" | undefined {
  if (!b) return undefined;
  if (BRIGHTNESS_RAW_SET.has(b)) {
    return b as "庙" | "旺" | "得" | "利" | "平" | "不" | "陷";
  }
  return undefined;
}
...
export function generateChart(birthInfo: BirthInfo): ZiweiChart {
  ...
  // round 4 H-2 修复：generateChart 入口校验（之前 hour=-1 / hour=13 静默接受，
  // 返回看似正常但语义错乱的命盘 — SLOP-09 等级生产事故）。
  if (!Number.isInteger(hour) || hour < 0 || hour > 12) {
    throw new Error(
      `hour must be integer 0-12 (时辰 branch index, 0=早子 ... 11=亥 ... 12=晚子), got ${hour}`,
    );
  }
  if (!Number.isInteger(month) || month < 1 || month > 12) {
    throw new Error(`month must be integer 1-12, got ${month}`);
  }
  if (!Number.isInteger(day) || day < 1 || day > 31) {
    throw new Error(`day must be integer 1-31, got ${day}`);
  }
  ...
  const palaces: Palace[] = astrolabe.palaces.map((p) => {
    ...
    const allStars: Star[] = [
      ...(p.majorStars ?? []).map((s) => ({
        ...
        brightnessRaw: mapBrightnessRaw(s.brightness as string), // round 5 NR5-2: 7 级原始亮度
  ...
  palaces.forEach((p) => {
    if (
      p.daXianAge &&
      currentAge >= p.daXianAge[0] &&
      currentAge <= p.daXianAge[1]
    ) {
  ...
  // ── 关键宫支 ──
  const mingGongBranch = BRANCHES.indexOf(
    astrolabe.earthlyBranchOfSoulPalace as string,
  );
  const shenGongBranch = BRANCHES.indexOf(
    astrolabe.earthlyBranchOfBodyPalace as string,
  );
  ...
  // round 4 M-1 卖点接通：先组装 baseChart，再喂给 patterns/sihua 计算。
  // detectPatterns(chart) 签名要求传入完整 ZiweiChart（patterns.ts:1018）；
  // getSiHuaByStem(stemIndex) 签名是数字 0-9 → Record<SiHua, string>（sihua.ts:18）。
  ...
  const patterns = detectPatterns(baseChart); // 34 detectors
  const sihua = getSiHuaByStem(lunarInfo.yearStem); // 倪师只算本命年干四化
